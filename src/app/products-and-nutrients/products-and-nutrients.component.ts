...
exclude(typeExcluded: string){
  ...
  else if (this.typeExcluded.indexOf('Исключить БАД')>=0) this.products.map((p:any)=>{if(p.val==0) p.excluded=String(p.fastdegree||'').startsWith('БАД') || p.group==='БАД'})
  else if (this.typeExcluded.indexOf('Только БАД')>=0) this.products.map((p:any)=>{if(p.val==0) p.excluded=!(String(p.fastdegree||'').startsWith('БАД') || p.group==='БАД')})
