...
function scoreMatch(queryRaw, entry) {
  ...
  // Prefer whole-word / prefix hits (гречка → крупа гречневая, not треска).
  for (const w of words) {
    if (w === q || w === qCyr) return 950;
    if (w.startsWith(q) || w.startsWith(qCyr)) return 900;
    if (q.length >= 4 && w.startsWith(q.slice(0, -1))) return 860;
    if (q.length >= 5 && w.startsWith(q.slice(0, -2))) return 840;
    if (q.length >= 4 && (w.includes(q) || q.startsWith(w))) return 820;
  }
  ...
  // Short queries: only allow very close edit distance on a single word.
  if (q.length <= 6) {
    let bestD = 99;
    for (const w of words) {
      if (Math.abs(w.length - q.length) > 2) continue;
      bestD = Math.min(bestD, levenshtein(w, q));
    }
    if (bestD <= 1) return 420 - bestD * 40;
    return 0;
  }
...
/**
 * Apply free-form product line to the layout (append to existing).
 * Fuzzy name match; missing qty → defaultAddPortion.
 * If product already in layout — increase its quantity.
 * @param {string} raw
 * @param {number} days
 * @param {Array} [existingMatched]
 * @returns {{items:string, found:number, missing:string[], added:number}}
 */
function applyFreeLayoutLine(raw, days, existingMatched) {
  const parsed = parseFreeLayoutLine(raw);
  if (!parsed.length) {
    return {
      items: buildLayoutItemsParamFromMatched(existingMatched || []),
      found: 0,
      missing: [],
      added: 0,
    };
  ...
  if (!matchIndex.length) buildMatchIndex();
  const matched = (existingMatched || []).map((row) => ({
  ...
  return {
    items: buildLayoutItemsParamFromMatched(matched),
    found: added,
    missing,
    added,
  };
...
function formatCalorieOnlyMessage(balanced, targetDays, hadAdditions) {
  ...
  if (!hadAdditions) {
    parts.push(
      'Продукты из рекомендаций не добавлялись. Если ниже остался дефицит — отметьте нужные позиции и снова нажмите «Добавить в раскладку».'
    );
  }
  return parts.join(' ');
}
...
/**
 * Standalone HTML snapshot of the current layout — same green/pink nutrient
 * colours as on the page, without changing the live UI chrome.
 */
function downloadLayoutReport(opts) {
  const o = opts || {};
  const matched = o.matched || [];
  const gaps = o.gaps || [];
  const days = o.days != null ? o.days : 1;
  const fastdegree = o.fastdegree || '';
  const trailOnly = !!o.trailOnly;
  const itemsRaw = o.itemsRaw || '';
  const shareUrl = o.shareUrl || '';
  const coveragePct = o.coveragePct;
  ...
  for (const it of matched) {
    if (!it || !it.product || !(it.grams > 0)) continue;
    const unit = it.product.section === 'bad' ? ' шт.' : ' г';
    productsRows +=
      '<tr><td>' + escapeHtml(it.product.name) +
      (it.product.section === 'bad' ? ' <span class="badge">БАД</span>' : '') +
      '</td><td class="num">' + it.grams + unit + '</td></tr>';
  }
  if (!productsRows) {
    productsRows = '<tr><td colspan="2">Нет продуктов с количеством</td></tr>';
  }
  ...
  for (const g of gaps) {
    ...
    const cls = (ok ? 'ok' : 'low') + (isCal ? ' cal' : '');
    nutrRows +=
      '<tr class="' + cls + '"><td>' + escapeHtml(g.n.name) +
      (isCal ? ' <span class="badge">главное</span>' : '') + '</td>' +
  ...
  const metaBits = ['срок ' + formatDays(days) + ' сут.'];
  if (fastdegree) metaBits.push('пост: ' + fastDegreeLabel(fastdegree));
  else metaBits.push('пост: все продукты');
  if (trailOnly) metaBits.push('походный запас');
  if (coveragePct != null && Number.isFinite(coveragePct)) {
    metaBits.push('покрытие ≈ ' + Math.round(coveragePct) + '%');
  }

  const css =
    'body{font-family:Georgia,"Times New Roman",serif;color:#1c2420;background:#faf7f1;' +
    'margin:0;padding:1.25rem;line-height:1.4}' +
    'h1{margin:0 0 .35rem;font-size:1.35rem;color:#2f5d50}' +
    'h2{margin:1.25rem 0 .4rem;font-size:1.05rem;color:#2f5d50}' +
    '.meta{color:#5a6560;font-size:.9rem;margin:0 0 1rem}' +
    'table{width:100%;border-collapse:collapse;font-size:.9rem;margin:.35rem 0 .75rem}' +
    'th,td{padding:.3rem .4rem;border-bottom:1px solid #ddd4c4;text-align:left;vertical-align:top}' +
    'th{color:#5a6560;font-size:.78rem}' +
    'td.num{text-align:right;font-variant-numeric:tabular-nums;white-space:nowrap}' +
    'tr.ok{background:#e7f5ec}' +
    'tr.low{background:#fce9e9}' +
    'tr.cal{box-shadow:inset 0 0 0 2px #2f5d50;font-weight:600}' +
    'tr.cal.ok{background:#d5f0e0}' +
    'tr.cal.low{background:#f8e4e4}' +
    '.badge{display:inline-block;font-size:.72rem;font-weight:700;padding:.1rem .4rem;' +
    'border-radius:999px;background:#d8f0e4;color:#1f6b4a;vertical-align:middle}' +
    '.note{color:#5a6560;font-size:.84rem;margin:.5rem 0}' +
    'code{font-size:.82em;background:#f0ebe1;padding:.05rem .3rem;border-radius:4px;' +
    'word-break:break-all}' +
    '.banner{margin:.5rem 0 .75rem;padding:.65rem .85rem;border-radius:8px;' +
    'border:1px solid #c5d9cc;background:#eef7f1;color:#1c3d32;font-size:.9rem}';

  const html =
    '<!DOCTYPE html><html lang="ru"><head><meta charset="utf-8"/>' +
    '<meta name="viewport" content="width=device-width,initial-scale=1"/>' +
    '<title>Раскладка — Диетолог v' + SEED.version + '</title>' +
    '<style>' + css + '</style></head><body>' +
    '<h1>Раскладка продуктов</h1>' +
    '<p class="meta">Диетолог простой v' + SEED.version + ' · ' +
    escapeHtml(stamp.toLocaleString('ru-RU')) + ' · ' + escapeHtml(metaBits.join(' · ')) +
    '</p>' +
    '<div class="banner">Снимок раскладки (тот же стиль таблицы нутриентов: зелёный — норма, ' +
    'розовый — дефицит). Файл можно открыть офлайн или распечатать.</div>' +
    '<h2>Продукты</h2>' +
    '<table><thead><tr><th>Продукт</th><th>Кол-во</th></tr></thead><tbody>' +
    productsRows + '</tbody></table>' +
    '<h2>Нутриенты на ' + escapeHtml(formatDays(days)) + ' сут.</h2>' +
    '<table><thead><tr><th>Нутриент</th><th>Есть</th><th>Нужно</th><th>Норма/сут</th>' +
    '<th>Дефицит</th><th>%</th></tr></thead><tbody>' + nutrRows + '</tbody></table>' +
    (shareUrl
      ? ('<h2>Ссылка для повторного открытия</h2><p class="note"><code>' +
        escapeHtml(shareUrl) + '</code></p>')
      : '') +
    (itemsRaw
      ? ('<p class="note"><b>items=</b> <code>' + escapeHtml(itemsRaw) + '</code></p>')
      : '') +
    '</body></html>';
  ...
  const a = document.createElement('a');
  a.href = url;
  a.download = 'dietolog-layout-' + fileStamp + '.html';
  document.body.appendChild(a);
  a.click();
  a.remove();
  setTimeout(() => {
...
function renderLayoutMode(panel, itemsRaw, daysFromQuery, fastFromQuery, trailFromQuery) {
  ...
  html += '<p class="mode-note" id="layoutShareNote"><b>Ссылка (для копирования / когда снова будет сеть):</b><br/>' +
    ...
    'Все расчёты и «Добавить в раскладку» работают <b>локально в этом файле</b> (без запроса к серверу).</p>';
  ...
  const dlBtn = box.querySelector('#btnDownloadLayout');
  if (dlBtn) {
  ...
  if (quickInput) {
    quickInput.addEventListener('keydown', (ev) => {
      ...
      if (!line) return;
      const result = applyFreeLayoutLine(line, targetDays, matched);
      const status = document.getElementById('status');
      if (!result.added && result.missing.length) {
        if (status) {
      ...
      let msg = 'Добавлено из строки: ' + result.added + ' продукт(ов)';
  ...
  function renderRecommendations() {
    ...
    let rhtml = '<div class="rec-head">' +
      ...
      'title="Добавить отмеченное в раскладку, пересчитать локально и обновить ссылку">' +
